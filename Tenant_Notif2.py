...
def insert_message_to_tables(sender, recipient, subject, message, attachment=None):
    ...
    try:
        # Insert into notif_sent_reply table (message_id is auto-generated)
        c.execute(''' 
            INSERT INTO notif_sent_reply (
                sender, recipient, subject, message, attachment, timestamp_sent_reply
            ) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        ''', (sender, recipient, subject, message, attachment))

        # Insert into notif_inbox table (message_id is auto-generated)
        c.execute(''' 
        ...
        # Fetch the latest inserted message_id
        c.execute("SELECT message_id FROM notif_sent_reply ORDER BY message_id DESC LIMIT 1")
        message_id = c.fetchone()[0]
        print(f"Inserted message into notif_sent_reply with ID: {message_id}")

        c.execute("SELECT message_id FROM notif_inbox ORDER BY message_id DESC LIMIT 1")
        message_id_inbox = c.fetchone()[0]
        print(f"Inserted message into notif_inbox with ID: {message_id_inbox}")
...
def send_message(Tenant_ID):
    subject = Subject.get()  # Assuming 'Subject' is a Tkinter Entry widget
    message = message_text.get(1.0, tk.END).strip()  # Assuming 'message_text' is a Tkinter Text widget
    attachment_path = AttachmentPath.get()  # Assuming 'AttachmentPath' is an Entry widget for file paths
    ...
    conn_rental = sqlite3.connect('db_messages6.db')
    c_rental = conn_rental.cursor()

    try:
        # Get the current timestamp for when the message is received
        timestamp_receive = current_time  # Could be the same as when sent if needed
        timestamp_read = None  # Assuming messages are unread by default
        status = 'sent'  # Assuming status is 'sent' when sending the message

        # Assuming you want to auto-generate message_id (e.g., autoincrement in DB)
        c_rental.execute("""
            INSERT INTO notif_inbox (sender, recipient, subject, message, attachment, timestamp_receive, timestamp_read, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (Tenant_ID, 'admin', subject, message, attachment_path, timestamp_receive, timestamp_read, status))

        conn_rental.commit()
        messagebox.showinfo("Success", "Message sent successfully!")

        # Close the compose window after successful send
        compose_win.destroy()  # Ensure 'compose_win' is correctly referenced

        # Refresh the sent messages view
        update_message_display(Tenant_ID)  # Ensure this function is defined
